...
struct Grid
    ...
    function Grid(intervals::Matrix{Float64}, points_per_var::Int)
        ...
        points = Matrix{Float64}(undef, points_per_var, n_var)
        for k in 1:n_var
            points[:, k] = collect(range(intervals[k][1], stop=intervals[k][2], length=points_per_var))
        end
        new(intervals, points, points_per_var, n_var)
    end

    function Grid(intervals::Matrix{Float64}, points_per_var::Vector{Int})
        error("Grid with different number of points per variables is not implemented yet.")
    end
